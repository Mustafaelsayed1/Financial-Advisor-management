...
# ✅ Configure Gemini
genai.configure(api_key="AIzaSyDqsJjBD4jbIsdF9pS8ZGnUShPOTKveC6M")  # Make sure you add your API key here
model = genai.GenerativeModel("gemini-1.5-flash")

# ✅ In-memory "database" to store user advice
user_advice_history = {}
...
def generate():
    try:
        ...
        try:
            ...
            print("⚠️ Failed to parse as JSON:", parse_err)
            result_json = {"output": result_text}

        # ✅ Save user input + model output into history
        user_advice_history[user_id] = {
            "input": {
                "income": income,
                "rent": rent,
                "utilities": utilities,
                ...
                "customExpenses": custom_expenses,
            },
            "output": result_json
        }
...
@app.route("/advice/last/<user_id>", methods=["GET"])
def get_last_advice(user_id):
    try:
        advice = user_advice_history.get(user_id)

        if not advice:
            print(f"⚠️ No advice found for user_id {user_id}")
            return jsonify({
                "message": "No previous advice found. Please submit the form first."
            }), 200  # ✅ 200 OK, not 404 error

        return jsonify(advice)
    except Exception as e:
        print("❌ Error fetching last advice:", e)
        return jsonify({"error": str(e)}), 500
